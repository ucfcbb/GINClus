...
def generate_align_length_threshold(cluster_pairwise_alignment_details):
    if align_len_threshold_type == 'length':
        ...
        if len(align_len_list) == 0:
            mean = std =0
            
        align_len_threshold = int(round(mean + std * align_len_zscore_threshold))
        # align_len_threshold = float(round(mean + std * align_len_zscore_threshold))
...
def merge_and_order_connected_components(connected_components_features, cluster_pairwise_alignment_details, align_len_threshold, is_normalized_score, generate_subfamilies):
    merge_all = not generate_subfamilies
            ...
            if i not in component_merged:
                for j in range(i+1, len(connected_components_features)):
                    if j not in component_merged:
                        ...
                        if (is_mergable1 and is_mergable2) or (connectivity_direction == "one-way" and (is_mergable1 or is_mergable2)) or merge_all:
                            ...
                            if component_merge_avg_RMSD <  best_component_merge_avg_RMSD or merge_all:
...
# def get_align_to_rmsd_info(cluster_id, cluster_pairwise_alignment_details, alignment_data, generate_subfamilies, fp_align_len_threshold = None):
def get_align_to_rmsd_info(cluster_id, cluster_pairwise_alignment_details, alignment_data, generate_subfamilies):
    global scanx_align_to_superimposition
    prev_status = scanx_align_to_superimposition
    scanx_align_to_superimposition = True
    ordered_dependency_list, merged_components_features, edges_among_all_components, edges_in_merged_components = generate_loop_print_dependency_v2(cluster_id, cluster_pairwise_alignment_details, alignment_data, generate_subfamilies)
    # ordered_dependency_list, merged_components_features, edges_among_all_components, edges_in_merged_components = generate_loop_print_dependency_v2(cluster_id, cluster_pairwise_alignment_details, alignment_data, generate_subfamilies, fp_align_len_threshold)
...
# def generate_loop_print_dependency_v2(cluster_id, cluster_pairwise_alignment_details, alignment_data, generate_subfamilies, fp_align_len_threshold = None):
def generate_loop_print_dependency_v2(cluster_id, cluster_pairwise_alignment_details, alignment_data, generate_subfamilies):
    print(cluster_id)
    # print(cluster_pairwise_alignment_details)
    ...
    component_ordering = merge_and_order_connected_components(connected_components_features, cluster_pairwise_alignment_details, align_len_threshold, is_normalized_score, generate_subfamilies)
...
def write_alignments_to_file(i, r1, j, r2, aln1, aln2, rmsd, parent_load_id, load_id, f, input_index_type):
    ###########################################################
...
def write_alignments_with_interactions_to_file(load_id, r, aln, rmsd, pdb_res_map_dict, f, input_index_type, categorize_annotation = True):
    ...
    f_loop = open(os.path.join(loop_dir, r.replace(':', '_') + ".smf"))
...
# def generate_subfamily_representative(fp_representative, cluster_id, component_id, component, alignment_data, rmsd_data_list_dict, pdb_res_map_dict, align_len_threshold, input_index_type):
def generate_subfamily_representative(cluster_id, component_id, component, alignment_data, rmsd_data_list_dict, pdb_res_map_dict, align_len_threshold, input_index_type):
    ...
    # write_alignments_with_interactions_to_file("", representative_loop, "", best_weighted_avg_rmsd, pdb_res_map_dict, fp_representative, input_index_type, False)
...
def generate_componentwise_analysis_files(superimposition_output_dir, cluster_id, ordered_dependency_list, load_id_dict, alignment_data, rmsd_data, input_index_type):
        ...
        for component_loop_id, ((i, r1), parent) in enumerate(component):
            ...
            if is_first == True:
                ...
                if parent != None:
                    ...
                    (t1, t2, zscore, cr1, cr2, aln2, aln1, score) = alignment_data[cluster_id][strToNode(r2)][strToNode(r1)]
                    rmsd = extract_rmsd_from_dict(rmsd_data_list_dict, i, r1, j, r2)
                    write_alignments_to_file(i, r1, j, r2, aln1, aln2, rmsd, parent_load_id, load_id, f3, input_index_type)
            else:
            # if parent != None:
                (j, r2) = parent
                parent_load_id, parent_loop_id = load_id_dict[(j, r2)]
                ...
                (t1, t2, zscore, cr1, cr2, aln2, aln1, score) = alignment_data[cluster_id][strToNode(r2)][strToNode(r1)]
                rmsd = extract_rmsd_from_dict(rmsd_data_list_dict, i, r1, j, r2)
                write_alignments_to_file(i, r1, j, r2, aln1, aln2, rmsd, parent_load_id, load_id, f1, input_index_type)

                ###########################################################
                #*************************r2******************************#
                write_alignments_with_interactions_to_file(parent_load_id, r2, aln2, rmsd, pdb_res_map_dict, f2, input_index_type)
                write_alignments_with_interactions_to_file(parent_load_id, r2, aln2, rmsd, pdb_res_map_dict, f4, input_index_type, False)

                #*************************r2******************************#
                f2.write('\n')
                f4.write('\n')
                write_alignments_with_interactions_to_file(load_id, r1, aln1, rmsd, pdb_res_map_dict, f2, input_index_type)
                write_alignments_with_interactions_to_file(load_id, r1, aln1, rmsd, pdb_res_map_dict, f4, input_index_type, False)
...
def generate_componentwise_bp_annotation_files(subfamily_details_dir, cluster_id, ordered_dependency_list, load_id_dict):
    ...
    for component_id, component in enumerate(ordered_dependency_list):
        ...
        for component_loop_id, ((i, r1), parent) in enumerate(component):
            ...
            f_loop = open(os.path.join(loop_dir, r1.replace(':', '_') + ".smf"))
            loop_info_lines = f_loop.readlines()
            f_loop.close()
            ...
            write_alignments_with_interactions_to_file("", r1, "", 0.0, pdb_res_map_dict, fw, input_index_type, False)
            fw.write('\n\n')
...
def load_pdb_in_pymol(partial_pdbx_dir, pdb_chain, pdb_align_ind_list, pdb_disp_ind_list, backbone_atom_list, sugar_atom_list, load_name_id, is_cif, loop_name):
    ...
    if is_cif:
        ...
        pymol.cmd.load(os.path.join(partial_pdbx_dir, loop_name.replace(':', '_')+'.cif'), pdb_load_name)
    else:
...
def get_pdb_residues(pdb_dir, pdb_chain, residue_list, structures, is_cif, loop):
    ...
    if loop in structures:
        ...
        pdb_fn = None
        if is_cif:
            pdb_fn = os.path.join(pdb_dir, str(strToNode(loop)).replace(':', '_') + '.cif')
...
def load_view_file(ordered_dependency_list, input_index_type):
...
def get_view_from_user(partial_pdbx_dir, cluster_id, ordered_dependency_list, loop_boundary_dict, loop_boundary_original_dict, input_index_type, show_extended_loop, is_cif):    
    if len(ordered_dependency_list) < 1 or len(ordered_dependency_list[0]) < 1:
...
def generate_formatted_superimposition_details(superimposition_details_dir, cluster_id, prev_text_name, pdb_organism_details, input_index_type):
...
def generate_pymol_images(time_in_distance_calc, removable_text_file_list, partial_pdbx_dir, superimposition_output_dir, subfamily_details_dir, superimposition_details_dir, pymol_session_dir, current_rmsd_data_dict, alignment_data, pdb_organism_details, loop_type, set_view_manually, draw_figures, show_extended_loop, generate_subfamilies, input_index_type, is_cif=True):

    # print("Using this generate pymol images")
    ...
    for cluster_id in sorted(current_rmsd_data_dict):
        ...
        if set_view_manually == True:
            get_view_from_user(partial_pdbx_dir, cluster_id, ordered_dependency_list, loop_boundary_dict, loop_boundary_original_dict, input_index_type, show_extended_loop, is_cif)
            continue
        
        r1_view = None
        if draw_figures == True:
            r1_view = load_view_file(ordered_dependency_list, input_index_type)

        generate_componentwise_bp_annotation_files(subfamily_details_dir, cluster_id, ordered_dependency_list, load_id_dict)
        if output_env == 'local':
            generate_componentwise_analysis_files(superimposition_output_dir, cluster_id, ordered_dependency_list, load_id_dict, alignment_data, current_rmsd_data_dict, input_index_type) 
        ...
        # avg_rmsd_align_to, total_alignment_length_align_to = get_align_to_rmsd_info(cluster_id, cluster_pairwise_alignment_details, alignment_data, generate_subfamilies)
        ...
        for component_id, component in enumerate(ordered_dependency_list):
            ...
            # represetative_i, representative_loop = generate_subfamily_representative(fp_representative, cluster_id, component_id, component, alignment_data, cluster_pairwise_alignment_details, pdb_res_map_dict, generate_align_length_threshold(cluster_pairwise_alignment_details), input_index_type)
            # represetative_i, representative_loop = generate_subfamily_representative(cluster_id, component_id, component, alignment_data, cluster_pairwise_alignment_details, pdb_res_map_dict, generate_align_length_threshold(cluster_pairwise_alignment_details), input_index_type)
            ...
            if draw_figures == True:
                if save_pymol_session == True:
                    ...
                    pymol.cmd.save(os.path.join(pymol_session_dir, str(cluster_id) + '.pse'))
                    pymol.cmd.sync()
        ...
        fp_text_fname.close()
        # generate_formatted_superimposition_details(superimposition_details_dir, cluster_id, text_fname, pdb_organism_details, input_index_type)
        ...
        logger.info('No. of loops: ' + str(len(loop_list)))
    ...
    for cluster_id in sorted(cluster_image_file_list):
        for rotation_version in sorted(cluster_image_file_list[cluster_id]):
            ...
            create_combined_subfamily_collage(os.path.join(subfamily_out_dir, 'Subfamily_Combined'), image_file_list, cluster_id, draw_figures, '')
...
#             pymol_session_name = str(cluster_id) + '.pse'
