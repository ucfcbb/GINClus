...
def load_cluster_alignment_data(alignment_data, clusters):
            ...
            for j in range(len(clusters[c_id])):
                ...
                if node1 not in alignment_data or node2 not in alignment_data[node1]:
                    ...
                    if node1 not in alignment_data:
                        if node1 not in problematic_tmalign_loops:
                            problematic_tmalign_loops.append(node1)
                        continue
                    if node2 not in alignment_data[node1]:
                        if node2 not in problematic_tmalign_loops:
                            problematic_tmalign_loops.append(node2)
                        continue
    ...
    # # print(problematic_tmalign_loops)
    # sys.exit()
    return cluster_alignment_data, get_loops_in_cluster(clusters)

def load_TM_alignment_data(input_fname_base, alignment_dir, alignment_tool, graphs_and_pickles_dir, alignment_fname, clusters, previous_graph_file_reused):

    alignment_data_fname = os.path.join(graphs_and_pickles_dir, alignment_tool + '_alignment_data_' + input_fname_base + '.pickle2')
    if (sys.version_info >= (3, 0)):
        alignment_data_fname = os.path.join(graphs_and_pickles_dir, alignment_tool + '_alignment_data_' + input_fname_base + '.pickle3')

    if previous_graph_file_reused == True:
        
        if is_valid_pickle(alignment_data_fname, clusters, alignment_tool) == True:
    ...
    logger.info('Reading TM-align alignment data from alignment files.\n')
    ...
    # for fn in glob.glob(os.path.join(alignment_dir, '*.aln')):
            fn = os.path.join(alignment_dir, r1.replace(':', '_') + '.aln')
...
def load_alignment_data(input_fname_base, alignment_dir, alignment_tool, graphs_and_pickles_dir, alignment_fname, clusters, previous_graph_file_reused):
    ...
    logger.info('Reading scanx alignment data from alignment files.\n')
    start_time = time.time()
...
def generate_rmsd_data(input_fname_base, partial_pdbx_dir, graphs_and_pickles_dir, alignment_data, alignment_tool, clusters, loop_list, previous_graph_file_reused):

    rmsd_data_fname = os.path.join(graphs_and_pickles_dir, alignment_tool + '_rmsd_data_' + input_fname_base + '.pickle2')
    if (sys.version_info >= (3, 0)):
        rmsd_data_fname = os.path.join(graphs_and_pickles_dir, alignment_tool + '_rmsd_data_' + input_fname_base + '.pickle3')
    
    # print('previous_graph_file_reused: ', previous_graph_file_reused)
    if previous_graph_file_reused == True:
        if is_valid_pickle(rmsd_data_fname, clusters, alignment_tool) == True:
...
def get_loop_string_for_filtering_log(r, input_index_type):
    loop_str = ''
...
def debug_print(r1, r2, rmsd, zscore, score, align_length, t1, t2, pairwise_align_details, aln_data, align_len_threshold, input_index_type):
    ...
    # if True:
        ...
        logger.info(get_loop_string_for_filtering_log(r1, input_index_type))
        logger.info(get_loop_string_for_filtering_log(r2, input_index_type))
        logger.info('zscore: ' + str(zscore) + ', rmsd: ' + str(rmsd) + ', alignment_score: ' + str(score) + ', alignment_length: ' + str(align_length))
        logger.info(get_loop_string_for_filtering_log(t1, input_index_type))
        logger.info(get_loop_string_for_filtering_log(t2, input_index_type))
        ...
        logger.info(get_loop_string_for_filtering_log(max_t1, input_index_type))
        logger.info(get_loop_string_for_filtering_log(max_t2, input_index_type))
...
def filter_loops_in_cluster(clusters, rmsd_data_dict, alignment_data, is_alignment_from_user, input_index_type):
            ...
            for (i, r1) in cluster_pairwise_align_details:
                if r1 in loops:
                    ...
                    if extreme_filtering == True and not is_acceptable_align_len(align_length, align_len_threshold):
                        logger.info('Filtering ' + get_loop_string_for_filtering_log(r1, input_index_type).ljust(75) + ' from ' + str(cluster_id).ljust(max_cid_len) + ' based on length \t(align_length: ' + str(align_length) + ') [Extreme filtering].')
                        removed_loops += 1
                    
                    elif extreme_filtering_based_on_rmsd(rmsd, align_length):
                        logger.info('Filtering ' + get_loop_string_for_filtering_log(r1, input_index_type).ljust(75) + ' from ' + str(cluster_id).ljust(max_cid_len) + ' based on rmsd \t(zscore: ' + "{:.3f}".format(round(zscore, 3)) + ', rmsd: ' + "{:.3f}".format(round(rmsd, 3)) + ') [Extreme filtering].')
                        removed_loops += 1
                    
                    elif remove_based_on_zscore(zscore, rmsd, align_length, is_alignment_from_user):
                        logger.info('Filtering ' + get_loop_string_for_filtering_log(r1, input_index_type).ljust(75) + ' from ' + str(cluster_id).ljust(max_cid_len) + ' based on zscore\t(zscore: ' + "{:.3f}".format(round(zscore, 3)) + ', rmsd: ' + "{:.3f}".format(round(rmsd, 3)) + ').')
                        removed_loops += 1

                    elif remove_based_on_rmsd(rmsd, align_length):
                        logger.info('Filtering ' + get_loop_string_for_filtering_log(r1, input_index_type).ljust(75) + ' from ' + str(cluster_id).ljust(max_cid_len) + ' based on rmsd \t(zscore: ' + "{:.3f}".format(round(zscore, 3)) + ', rmsd: ' + "{:.3f}".format(round(rmsd, 3)) + ').')
                        removed_loops += 1
...
def load_alignment_and_rmsd_data(clusters, loop_list, input_fname_base, partial_pdbx_dir, alignment_dir, alignment_tool, graphs_and_pickles_dir, previous_graph_file_reused):
    graph_fname = os.path.join(graphs_and_pickles_dir, input_fname_base + '_' + alignment_tool + '.z.graph')


    if alignment_tool == 'tmalign':
        alignment_data = load_TM_alignment_data(input_fname_base, alignment_dir, alignment_tool, graphs_and_pickles_dir, graph_fname, clusters, previous_graph_file_reused)
    else:
        alignment_data = load_alignment_data(input_fname_base, alignment_dir, alignment_tool, graphs_and_pickles_dir, graph_fname, clusters, previous_graph_file_reused)
    ...
    rmsd_data_dict = generate_rmsd_data(input_fname_base, partial_pdbx_dir, graphs_and_pickles_dir, alignment_data, alignment_tool, clusters, loop_list, previous_graph_file_reused)
...
def generate_superimposition_images(clusters, loop_list, alignment_data, rmsd_data_dict, draw_figures, filter_cluster, set_view_manually, show_extended_loop, is_alignment_from_user, user_input_fname, removable_text_file_list, directories, loop_type, generate_subfamilies, input_index_type):
    ...
    (partial_pdbx_dir, subfamilies_dir, subfamily_details_dir, superimposition_output_dir, superimposition_details_dir, pymol_session_dir) = directories
    ...
    if filter_cluster:
        clusters = filter_loops_in_cluster(clusters, length_adjusted_rmsd_score_dict, alignment_data, is_alignment_from_user, input_index_type)
    ...
    time_in_distance_calc = generate_pymol_images(0, removable_text_file_list, partial_pdbx_dir, superimposition_output_dir, subfamily_details_dir, superimposition_details_dir, pymol_session_dir, current_rmsd_data_dict, alignment_data, pdb_organism_details, loop_type, set_view_manually, draw_figures, show_extended_loop, generate_subfamilies, input_index_type)
    ...
    for cluster_id in clusters:
        ...
        for i, loop in enumerate(loops):
            ...
            if i == 0:
                ...
                # sys.exit()
            pymol.cmd.load(os.path.join(partial_pdbx_dir, loop.replace(':', '_')+'.cif'), load_name)
