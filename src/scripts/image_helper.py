...
def cleanup_images(superimposition_output_dir, subfamilies_dir):
...
def get_cluster_code(image_name, out):
    ...
    return get_motif_family_short_code(family_name)
...
def get_image_and_captions(image_list, show_pdb_info, pdb_organism_details, out):
    ...
    for (loop, png, loop_count) in image_list:
        ...
        caption = cluster_code if len(cluster_code) < 15 else cluster_code[:14] + '.'
...
def create_combined_subfamily_collage(subfamily_dir, image_list, cluster_id, draw_figures, suffix = '', padding_dimension = 40):
    ...
    # print("Testing: ", cluster_id)
    # print(subfamily_dir)
    # cluster_id_no = cluster_id.split("_")[1]
    # out = os.path.join(subfamily_dir, str(cluster_id) + '_' + suffix + '.png')
    out = os.path.join(subfamily_dir, str(cluster_id) + '.png')
    # out = os.path.join('subcluster_' + str(cluster_id_no) + '.png')
...
# def create_collage(image_list, out, draw_text = True, text_padding = 20, dimension_thumb = 400, padding_dimension = 20, title_height = 0):
    ...
    # title, image_list = get_image_and_captions(image_list, out)
